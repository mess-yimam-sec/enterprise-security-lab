...
The goal was simple: I wanted GitHub Actions to work with AWS without
...
This started as a small federation exercise, but it became the identity
foundation for the Terraform CI/CD work that followed.

Security principle: GitHub Actions should receive temporary AWS
credentials through federation rather than storing permanent AWS
access keys as repository secrets.

Why I Used OIDC

Before setting up GitHub OIDC, I had already worked with IAM users, MFA,
AWS STS, and temporary credentials.

That helped me see an important difference between local access and
CI/CD access.

For a person working locally, an IAM identity can authenticate and
obtain temporary credentials. A GitHub Actions workflow is different. I
did not want to create an IAM user just for GitHub or store an access
key and secret access key in the repository.
...
The workflow receives AWS access only when it runs and only through the
IAM role that AWS allows it to assume.
...
GitHub manages the workload identity. AWS trusts GitHub's OIDC identity
provider and evaluates the claims in the token against an IAM role trust
policy.
...
I had already encountered an OAuth-based flow while experimenting with
aws login, where the CLI waited for a browser callback similar to:

http://127.0.0.1:<port>/oauth/callback

That earlier troubleshooting helped provide context for the GitHub
federation work.
...
For this lab, the important idea is that GitHub can issue a signed token
describing the identity of the workflow.
...
AWS validates that token and then checks whether the IAM role's trust
policy accepts the identity described by its claims.
...
That distinction became important later when I separated Terraform
planning and deployment permissions.
...
For the initial proof of concept, I created:

EnterpriseSecurityLab-GitHubOIDC

The role used the GitHub OIDC provider:
...
The role was intentionally limited rather than being given
AdministratorAccess.

For the initial federation test, it had read access to networking
information such as:
...
This gave me enough permission to prove that federation worked without
giving the workflow unnecessary administrative access.
...
The tags are organizational metadata. They are not part of the
...
This distinction helped reinforce the difference between identity and
authorization.

First OIDC Failure

The first workflow reached the AWS credential configuration step but
failed with:
...
That pointed me toward the IAM role's trust conditions rather than AWS
access keys or general GitHub workflow configuration.

Instead of broadening the trust policy until the workflow worked, I
wanted to understand exactly which claim AWS was rejecting.
...
These values gave me the information I needed to understand why the
role's subject condition did not match the GitHub identity.
...
This explained the AssumeRoleWithWebIdentity failure: AWS was
correctly rejecting a token whose subject did not match the role's trust
policy.
...
The subject restricts which GitHub identity may assume the role.

Successful Federation Test
...
and the resulting ARN identified an assumed role session in the general
form:

arn:aws:sts::ACCOUNT-ID:assumed-role/EnterpriseSecurityLab-GitHubOIDC/...

That was important because it proved the workflow was operating through
the IAM role rather than as the mess-bg-lab IAM user.
...
Earlier in the lab, I used an IAM-user-based path to obtain temporary
credentials:
...
OIDC
= Who is this external workload?

IAM role trust policy
= May this workload assume the role?
...
This separation became especially important as the Terraform pipeline
became more mature.
...
As the project moved from a branch-based OIDC proof of concept to
controlled Terraform deployment, the deployment workflow began using the
...
This is separate from the earlier branch-based proof-of-concept subject
used by EnterpriseSecurityLab-GitHubOIDC.
...
More importantly, the exercise became the authentication model for the
later Terraform CI/CD pipeline.
